...
class AppState: ObservableObject {
    ...
    @Published var navigationWidth: CGFloat = 150
    @Published var codeCanvasWidth: CGFloat = 220

    init() {
        // 初始化 nodeViewModels
        self.nodeViewModels = [
            NodeViewModel(node: Node(code: "Drink Water", position: CGPoint(x: 200, y: 200))),
            NodeViewModel(node: Node(code: "Have a nice day", position: CGPoint(x: 100, y: 100)))
        ]
        loadRootDirectories()
    }
